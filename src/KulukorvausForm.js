...
export default function KulukorvausForm() {
  ...
  const handleSubmit = async () => {
    ...
    // Formspree — sinulle + estiem (liitteineen)
    const formspreeData = new FormData();
    formspreeData.append("_subject", subject);
    formspreeData.append("_replyto", email);
    if (formType === "estiem") formspreeData.append("_cc", "estiem@kaplaaki.fi");
    Object.entries(sharedFields).forEach(([k, v]) => formspreeData.append(k, v));
    attachedFiles.forEach(f => formspreeData.append("liitteet", f, f.name));

    // Web3Forms — hakijalle (ei liitteitä)
    const web3Data = new FormData();
    web3Data.append("access_key", WEB3FORMS_KEY);
    web3Data.append("subject", `Kopio kulukorvausanomuksestasi — ${name}`);
    web3Data.append("from_name", "Kaplaaki ry");
    web3Data.append("to_email", email);
    Object.entries(sharedFields).forEach(([k, v]) => web3Data.append(k, v));

    setSending(true);
    setSendError(null);
    try {
      const [fsRes] = await Promise.all([
        fetch(`https://formspree.io/f/${FORMSPREE_ID}`, { method: "POST", body: formspreeData, headers: { Accept: "application/json" } }),
      ...
      if (fsData.ok) {
        ...
        setSendError("Lähetys epäonnistui: " + (fsData.error || "tuntematon virhe"));
      }
    } catch (err) {

...
export default function KulukorvausForm() {
  ...
  const handleSubmit = async () => {
    ...
    netlifyData.append("form-name", "kulukorvaus");
    netlifyData.append("bot-field", "");
    Object.entries(sharedFields).forEach(([k, v]) => netlifyData.append(k, v));
    attachedFiles.forEach(f => netlifyData.append("liitteet", f, f.name));

    // Web3Forms — hakijalle (kopio ilman liitteitä)
    const web3Data = new FormData();
    web3Data.append("access_key", WEB3FORMS_KEY);
    web3Data.append("subject", `Kopio kulukorvausanomuksestasi — ${subject}`);
